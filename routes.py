...
#runs to get to the resume page
@app.route('/resume')
def resume():
...
#runs to get to the projects page
@app.route('/projects')
def projects():
	
	return render_template('projects.html')

#runs to get to the piano page
@app.route('/piano')
def piano():
	
	return render_template('piano.html')

#runs to get to the update resume page
@app.route('/updateresume')
@login_required
def updateresume():
	return render_template('updateresume.html')

#runs to get to the resume options page
@app.route('/resumeoptions')
@login_required
def resumeoptions():
	return render_template('resumeoptions.html')

#runs to get the add to resume page
@app.route('/addresume')
@login_required
def addresume():
	return render_template('addresume.html')

#runs to get the pick which job to edit page
@app.route('/pickedit')
@login_required
def pickedit():
	resume_data = db.getResumeData()
	return render_template('pickedit.html', resume_data = resume_data)

#runs to get information on the desired job to be edited
@app.route('/editresume', methods = ['POST'])
@login_required
def editresume():
	selected_option = request.form.get("option")

	resume_data = db.getSpecificResumeData(selected_option)

	return render_template('editresume.html', resume_data = resume_data)

#runs to add the edited data to the database return to the resume page
@app.route('/editdone', methods = ['POST'])
@login_required
def editdone():
	...
	for key, value in request.form.items():
		if value:
			...
			lookUp = searchTerms[0].strip()
			col = searchTerms[1].strip()
			original = searchTerms[2].strip()
			...
			#adds to the database based on the parameter
			if len(searchTerms) == 3:
				db.query(f"UPDATE {lookUp} SET {col} = '{value.strip()}' WHERE {col} = '{original}'")

			
			elif len(searchTerms) == 4:
				institution = searchTerms[3].strip()
				db.query(f"UPDATE {lookUp} SET {col} = '{value.strip()}' WHERE {col} = '{original}' AND inst_id = '{institution}'")
	

	return redirect('/resume')

#runs to add a new job to the database return to the resume page
@app.route('/addjob', methods = ['POST'])
@login_required
def addjob():
	job = request.form

	#adds new institution to the database
	header = ["name", "city", "state"]
	information = [[]]
	information[0].append(job["name"])
	information[0].append(job["city"])
	information[0].append(job["state"])

	inst_id = db.insertRows("institutions", header, information)
	countPos = 0
	while True:
		

		try:
			#adds new position to database
			header = ["inst_id", "title", "responsibilities","start_date","end_date"]
			information = [[]]
			information[0].append(inst_id)
			information[0].append(job[str(countPos) +" | title"])
			information[0].append(job[str(countPos) + " | responsibilities"])
			information[0].append(job[str(countPos)+" | start_date"])
			information[0].append(job[str(countPos)+" | end_date"])
		

			position_id = db.insertRows("positions", header, information)
			...
			while True:
				...
				try:
					...
					#adds new experience to the database
					header = ["position_id", "name", "description"]
					information = [[]]
					information[0].append(position_id)
					information[0].append(job[str(countPos)+" | "+str(countExp) + " | experience"])
					information[0].append(job[str(countPos)+" | "+str(countExp) + " | experience-description"])
					experience_id = db.insertRows("experiences", header, information)

					#runs through each skill
					for skill in job[str(countPos)+" | "+str(countExp) + " | skills"].split(";"):
						header = ["experience_id", "name"]
						information = [[]]
						information[0].append(experience_id)
						information[0].append(skill)

						#adds new skill to the database
						db.insertRows("skills", header, information)
...
#runs to remove a job from the resume returns to the resume page
@app.route('/removejob', methods = ['POST'])
@login_required
def removejob():
	selected_option = request.form.get("option")

	resume_data = db.removeJob(selected_option)

	return redirect('/resume')

#runs to get the options for the jobs to remove
@app.route('/removeresume')
@login_required
def removeresume():
	resume_data = db.getResumeData()
	return render_template('removeresume.html', resume_data = resume_data)
